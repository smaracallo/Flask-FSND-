...
def show_venue(venue_id):
    ...
    db_query = db.session.query(Show).join(Artist).filter(Show.venue_id == venue_id)
    future = db_query.filter(Show.start_time <= datetime.now()).all()
    past = db_query.filter(Show.start_time > datetime.now()).all()
...
def search_artists():
    ...
    for result in search_results:
        data.append({
            'id': result.id,
            'name': result.name,
            'num_upcoming_shows': len(db.session.query(Show).filter(Show.artist_id == result.id).
                                      filter(Show.start_time > datetime.now()).all())
        })
    response = {
        'count': len(search_results),
...
def show_artist(artist_id):
    ...
    db_query = db.session.query(Show).join(Venue).filter(Show.artist_id == artist.id)
    artist_past_shows = db_query.filter(Show.start_time < datetime.now()).all()
    artist_upcoming_shows = db_query.filter(Show.start_time >= datetime.now()).all()
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        db.session.close()
        if error:
            flash(f'Error: Venue {venue.name} could not be updated.')
        else:
